...
## Regenerating the V2G EXI Codec

The ISO 15118 implementation relies on an EXI codec generated by
[cbexigen](https://github.com/EVerest/cbexigen).  The helper script
`tools/exi_codegen/generate_exi.sh` automates the generation.

1. Place the XSD schema files of DIN 70121 and ISO 15118‑2/‑20 inside
   `tools/exi_codegen/schemas/`.  ISO schemas can also be downloaded
   automatically by the script.
2. Run `tools/exi_codegen/generate_exi.sh`.  Set the environment variable
   `EXI_CONFIG_FILE=/path/to/config.py` to select a custom feature set via
   `--config_file`.
3. The script clones `cbexigen`, runs `python src/main.py` and copies the
   generated `c/` output into `lib/v2g_exi`.

Whenever new standard revisions become available, update the schema files
or configuration and rerun the script to regenerate the codec.
